            ...
            return """
                    WITH answer_metrics AS (
                        SELECT
                            ...
                            END * #{scoreWeight} + 
                            CASE WHEN a.is_accepted THEN #{acceptedWeight} ELSE 0 END) AS normalized_score
                        FROM answer a
                        INNER JOIN question q ON a.question_id = q.id
                        INNER JOIN "user" u ON a.owner_id = u.account_id
                        WHERE 
                            CASE WHEN #{minAnswerLength} IS NOT NULL 
                                ...
                                ELSE true END
                            AND
                            CASE WHEN #{maxAnswerLength} IS NOT NULL 
                                THEN LENGTH(REGEXP_REPLACE(
                                        REGEXP_REPLACE(
                                            REGEXP_REPLACE(a.body, '<[^>]+>', '', 'g'),
                                            '&[^;]+;', '', 'g'),
                                ...
                                ELSE true END
                            AND CASE WHEN #{maxTimeGap} IS NOT NULL 
                                THEN EXTRACT(EPOCH FROM (a.creation_date - q.creation_date)) <= #{maxTimeGap} 
                                ELSE true END
        ...
        public static String getTimeGapGroupsQuery(AnswerAnalysisParams params) {
            return getAnswerMetricsCTE(params) + """
                    SELECT
                        ...
                        MIN(time_gap)::integer as range_start,
                        MAX(time_gap)::integer as range_end,
                        ROUND(AVG(normalized_score)::numeric, 4) as avg_quality_score,
                        COUNT(*) as count,
        ...
        public static String getReputationGroupsQuery(AnswerAnalysisParams params) {
            return getAnswerMetricsCTE(params) + """
                    SELECT
                        ...
                        MIN(user_reputation)::integer as range_start,
                        MAX(user_reputation)::integer as range_end,
                        ROUND(AVG(normalized_score)::numeric, 4) as avg_quality_score,
                        COUNT(*) as count,
                        ROUND(AVG(CASE WHEN is_accepted THEN 1.0 ELSE 0.0 END)::numeric, 4) as accepted_rate
                    FROM answer_metrics
        ...
        public static String getLengthGroupsQuery(AnswerAnalysisParams params) {
            return getAnswerMetricsCTE(params) + """
                    SELECT
                        ...
                        MIN(answer_length)::integer as range_start,
                        MAX(answer_length)::integer as range_end,
                        ROUND(AVG(normalized_score)::numeric, 4) as avg_quality_score,
                        COUNT(*) as count,
                        ROUND(AVG(CASE WHEN is_accepted THEN 1.0 ELSE 0.0 END)::numeric, 4) as accepted_rate
                    FROM answer_metrics
